...
class Mall extends ModelActiveRecord
{
    ...
    public function getMallSetting($columns = [])
    {
        if ($this->mallAllOptions) {
            ...
            $newOption = [];
            $arr = ['add_app_bg_transparency', 'add_app_bg_radius', 'is_show_cart', 'is_show_goods_name', 'is_show_stock', 'is_sales', 'is_must_login', 'is_goods_video', 'is_open', 'open_type', 'is_auto_open', 'show_contact_type'];
            foreach ($detail['option'] as $k => $item) {
                if (in_array($item['key'], $arr)) {
                    $item['value'] = (int)$item['value'];
                }
                if (in_array($item['key'], ['good_negotiable', 'payment_type', 'send_type', 'business_time_type_week', 'business_time_type_day', 'quick_customize_new_params', 'send_type_desc', 'week_list', 'time_list'])) {
