...
class CommonShare extends Model
{
    ...
    public function becomeShareByPayed($order)
    {
        ...
        if ($shareCondition != 3) {
            ...
            if ($becomeCondition == 1) {
                ...
                return $this->becomeShareByTotalConsume($order);
            } else {
                return true;
            }
        }
     ...
     * 消费满额自动成为分销商
     */
    public function becomeShareByTotalConsume($order)
    {
        ...
        $orderCount = $commonMallMember->getOrderMoneyCount($mallId, $userId);
        $orderCount = floatval($orderCount);
