...
class OrderExport extends BaseExport
{
    ...
    protected function transform($list)
    {
        ...
        foreach ($list as $item) {
            $arr = [];
            $arr['platform'] = isset($item->user->userInfo->platform) ? $this->getPlatform($item->user->userInfo->platform) : '';
            ...
            if ($sign) {
                ...
                foreach ($item->detail as $index => $detailItem) {
                    ...
                    $newArr['original_price'] = (float)$detailItem->goods->goodsWarehouse->original_price;
                    $newArr['price'] = $detailItem->goods->price;
